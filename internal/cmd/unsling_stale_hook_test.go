...
func readHookSlotViaBD(workDir, agentBeadID string) (string, error) {
	...
	out, err := runUnslingTestCommand(workDir, "bd", "sql", "--json", q)
	if err != nil {
		return "", err
	}
...
func runUnslingTestCommand(workDir, name string, args ...string) ([]byte, error) {
	cmd := exec.Command(name, args...) //nolint:gosec // static command in tests
	cmd.Dir = workDir
